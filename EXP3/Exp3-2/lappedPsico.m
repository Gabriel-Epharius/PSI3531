function [y, SMRs, vetor_Nbits]=lappedPsico(x,fa,H,G,bitrate)
...
SMRs = zeros(Nframes/12,32);
vetor_Nbits =zeros(32,32);
...
for i=1:12:Nframes
    ...
    SMRs(ceil(i/12),:) = transpose(SMR);
    Nbits = MPEG1_bit_allocation(SMR, bitrate);
    vetor_Nbits(:, ceil(i/12)) = Nbits;
    for k=1:M
        if Nbits(k)~=0
